...
# hurt(DamageSource, float) does not receive a ServerLevel in 1.21.1
...
# Item particles still take an ItemStack
patch(
    "projectile/LuckyEggProjectile.java",
    lambda t: t.replace(
...
# Block item interaction uses ItemInteractionResult in 1.21.1
...
def fix_present_box(text: str) -> str:
    ...
    text = text.replace(
        "        return openPresent(level, pos, player);\n    }\n\n    private static InteractionResult openPresent",
...
# Item#use returns InteractionResultHolder<ItemStack> in 1.21.1
...
# InteractionHand had no asEquipmentSlot helper in 1.21.1
for rel in ["item/BloodyMaidenHeartItem.java", "item/HarpoonItem.java"]:
    def fix_slot(text: str) -> str:
        ...
        text = add_import(text, "import net.minecraft.world.entity.LivingEntity;\n" if "LivingEntity" in text else "import net.minecraft.world.InteractionHand;\n", "import net.minecraft.world.entity.EquipmentSlot;\n")
        return text.replace(
...
# Old Item#hurtEnemy returns boolean
...
# Entity spawn placement / builder signatures
...
if IS_FABRIC:
    ...
    def fix_fabric_items(text: str) -> str:
        ...
        text = text.replace(
            "new SpawnEggItem(properties.spawnEgg(type.get()))",
            "new SpawnEggItem(type.get(), 0xFFFFFF, 0xFFFFFF, properties)",
        )
